        ...
        return (
            <Card>
                <div style={{ marginBottom: '20px' }}>
                    ...
                    >
                        {formatMessage({
                            id: 'pages.clients.list.table.client.add',
                            defaultMessage: 'Add Client',
                        })}
                    </Button>
                </div>
                <Table dataSource={list} loading={loading} rowKey="clientId" pagination={false}>
                    <Table.Column
                        width={100}
                        dataIndex="logoUri"
                        title={formatMessage({ id: 'pages.clients.list.table.client.logo' })}
                        render={record => <Avatar src={record} />}
                    />
                    <Table.Column
                        dataIndex="clientId"
                        title={formatMessage({ id: 'pages.clients.list.table.client.id' })}
                    />
                    <Table.Column
                        dataIndex="clientName"
                        title={formatMessage({ id: 'pages.clients.list.table.client.name' })}
                    />
                    <Table.Column
                        dataIndex="enabled"
                        title={formatMessage({ id: 'pages.clients.list.table.client.enabled' })}
                        render={record =>
                            record ? (
                                <Badge status="success" text={formatMessage({ id: 'app.shared.enabled' })} />
                            ) : (
                                    <Badge status="error" text={formatMessage({ id: 'app.shared.disabled' })} />
                                )
                        }
                    />
                    <Table.Column
                        render={(text, record: IClientViewModel) => (
                            <>
                                <Button
                                    type="ghost"
                                    onClick={() => {
                                ...
                                <Popconfirm
                                    placement="topLeft"
                                    title={formatMessage({
                                        id: 'pages.clients.list.table.remove.confirm',
