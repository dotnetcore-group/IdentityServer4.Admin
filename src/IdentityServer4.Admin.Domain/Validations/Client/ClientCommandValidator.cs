...
{
    public abstract class ClientCommandValidation<T> : AbstractValidator<T> where T : ClientCommand
    {
        ...
        {
            RuleFor(c => c.Client.ClientId).NotNull().NotEmpty().WithMessage("ClientId cannot be null or empty");
        }

        protected void ValidateClientName()
        {
            RuleFor(c => c.Client.ClientName).NotNull().NotEmpty().WithMessage("ClientName cannot be null or empty");
        }
